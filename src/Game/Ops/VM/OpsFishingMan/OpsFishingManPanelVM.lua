...
---@class OpsFishingManPanelVM : UIViewModel
local OpsFishingManPanelVM = LuaClass(UIViewModel)
---Ctor
function OpsFishingManPanelVM:Ctor()
    self.TitleText = nil
...
function OpsFishingManPanelVM:Update(ActivityData)
    ...
    if NodeList and #NodeList > 0 then
        ...
        if Extra and ShareBuyNodeCfg then
            ...
            self.Discount = UIBinderSetTextFormatForMoney:GetText(ShareBuyNodeCfg.Params[1] - ShareBuyNodeCfg.Params[2])
            ...
            elseif self.ShareBuyData.Status == ProtoCS.Game.Activity.enStatus.OriginalPayed then
                ...
                local FinishNum, TotalNum = self:GetCashBackProgress(ActivityData)
                self.BuyText = string.format("%s(%d/%d)", LSTR(1470011), FinishNum, TotalNum)
                self.BuyTagText = LSTR(1470029)
                self.BuyPriceText = ""
            ...
            elseif self.ShareBuyData.Status == ProtoCS.Game.Activity.enStatus.DiscountPayed then
                --已购买-优惠
                self.BuyTagText = nil
                self.BuyText = LSTR(1470031)
                self.BuyActionVisible = false
                self.BuyPriceText = LSTR(1470030)
                self.BuyPriceVisible = false
                self.DiscountMoneyVisible = false
            end
...
function OpsFishingManPanelVM:GetShareBuyStatus()
...
function OpsFishingManPanelVM:GetCashBackProgress(ActivityData)
    local TotalNum = 0
    local FinishNum = 0
...
function OpsFishingManPanelVM:UpdatePlayState()
...
function OpsFishingManPanelVM:BPlayChecked()
	return self.PlayState == EToggleButtonState.Checked
end

function OpsFishingManPanelVM:UpdateSoundState()
...
function OpsFishingManPanelVM:BSoundChecked()
	return self.SoundState == EToggleButtonState.Checked
end


return OpsFishingManPanelVM