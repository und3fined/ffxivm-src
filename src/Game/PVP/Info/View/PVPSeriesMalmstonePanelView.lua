...
local PVPSeriesMalmstoneVM = require ("Game/PVP/Info/VM/PVPSeriesMalmstoneVM")
...
function PVPSeriesMalmstonePanelView:OnRegisterTimer()
...
function PVPSeriesMalmstonePanelView:OnSeriesDataChanged(NewValue, OldValue)
	...
	if CurVersionCfg then
		local ExpText = ""
		local Percent = 0
		if CurLevel >= CurVersionCfg.LevelMax then
			ExpText = LSTR(130067)
			Percent = 1
		else
			local CurExp = PVPInfoMgr:GetCurSeriesMalmstoneExp()
			local NeedExp = PVPInfoMgr:GetSeriesMalmstoneLevelUpExp(CurLevel)
...
function PVPSeriesMalmstonePanelView:OnIsSeriesOpeningChanged(NewValue, OldValue)
	local CurVersionCfg = PVPInfoMgr:GetCurVersionSeriesMalmstoneCfg()
	if CurVersionCfg then
		local Visible = CurVersionCfg.Season ~= 0
		UIUtil.SetIsVisible(self.TextSeason, Visible)
		UIUtil.SetIsVisible(self.ImgSeasonBg, Visible)

		if Visible then
			local Text = string.format(LSTR(130124), CurVersionCfg.Season)
			self.TextSeason:SetText(Text)
		end
	end
end
...
function PVPSeriesMalmstonePanelView:OnClickBtnTrophyCrystal()
...
function PVPSeriesMalmstonePanelView:OnTimer()
	local Visible = false
	local SeasonCfg = PVPInfoMgr:GetCurSeasonSeriesMalmstoneCfg()
	if SeasonCfg then
		local RemainTime = PVPInfoMgr:GetCurSeasonSeriesMalmstoneRemainTime()
		if RemainTime > 0 then
			local AtLeastDay = PVPInfoMgr:GetSeriesMalmstoneAtLeastShowDay()
			local AtLeastSecond = AtLeastDay * 24 * 60 * 60
			if RemainTime <= AtLeastSecond then
				Visible = true
		...
		if PVPInfoMgr:GetCurVersionSeriesMalmstoneCfg() then
			Visible = true
			self.TextTime:SetText(LSTR(130073))
		end
	end

	UIUtil.SetIsVisible(self.HorizontalTime, Visible)
end
