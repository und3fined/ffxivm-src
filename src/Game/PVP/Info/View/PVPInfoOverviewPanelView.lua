...
local PVPHonorItemVM = require ("Game/PVP/Info/VM/PVPHonorItemVM")
...
function PVPInfoOverviewPanelView:OnRegisterTimer()
...
function PVPInfoOverviewPanelView:OnSeriesDataChanged(NewValue, OldValue)
	...
	if CurSeasonCfg and CurVersionCfg then
		CurLevel = NewValue and NewValue.Level or 0
	...
	elseif CurVersionCfg then
		local ServerTime = TimeUtil.GetServerLogicTime()
		local BeginTime = TimeUtil.GetTimeFromServerZoneString(CurVersionCfg.StarRoadBeginTime)
		...
		if RemainTime > 0 then
			...
			CurLevel = NewValue and NewValue.Level or 0
			CurExp = PVPInfoMgr:GetCurSeriesMalmstoneExp()
		end
...
function PVPInfoOverviewPanelView:OnTimer()
	local Visible = false
	...
	if SeasonCfg then
		...
		local VersionCfg = PVPInfoMgr:GetCurVersionSeriesMalmstoneCfg()
		if VersionCfg then
			local ServerTime = TimeUtil.GetServerLogicTime()
			local BeginTime = TimeUtil.GetTimeFromServerZoneString(VersionCfg.StarRoadBeginTime)
			local RemainTime = math.ceil(BeginTime - ServerTime)
			if RemainTime > 0 then
				local TimeString = LocalizationUtil.GetCountdownTimeForLongTime(RemainTime)
				if TimeString then
					local Text = string.format(LSTR(130069), TimeString)
					self.RichTextSeriesRemainTime:SetText(Text)
					Visible = true
				end
			else
				self.RichTextSeriesRemainTime:SetText(LSTR(130072))
			end
		end
	end

	UIUtil.SetIsVisible(self.RichTextSeriesRemainTime, Visible)
end
