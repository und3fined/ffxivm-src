...
local PVPPerformanceDetailItemVM = require ("Game/PVP/Info/VM/PVPPerformanceDetailItemVM")
...
function PVPCrystallinePerformanceVM:ShowRankDetail()
    ...
    local CurSeasonWinRateText = string.format(LSTR(130057), CurSeasonWinRate)
    
    local CurRank = PVPInfoVM:GetCurCrystallineRank()
    local CurRankCfg = CrystallineRankCfg:FindCfgByKey(CurRank) or {}
    local CurRankStarVisible = CurRankCfg.ResultMode == ProtoRes.Game.pvp_rank_result_mode.RRM_WINSTAR
    local CurRankWinStar = CurRankCfg.WinStar
    local CurRankName = CurRankCfg.RankName or LSTR(130063)
    local CurRankNameText = ""
    if CurRankStarVisible then
        if CurRankCfg.Type ~= ProtoRes.Game.pvp_rank_type.RT_None then
            CurRankNameText = string.format(LSTR(130074), CurRankName)
        end
    else
        local CurScore = 0
        CurRankNameText = string.format(LSTR(130120), CurRankName, CurScore)
    end

    local HighestRank = PVPInfoVM:GetSeasonHighestCrystallineRank()
    local HighestRankCfg = CrystallineRankCfg:FindCfgByKey(HighestRank) or {}
    local HighestRankStarVisible = HighestRankCfg.ResultMode == ProtoRes.Game.pvp_rank_result_mode.RRM_WINSTAR
    local HighestRankWinStar = HighestRankCfg.WinStar
    local HighestRankName = HighestRankCfg.RankName or LSTR(130064)
    local HighestRankNameText = ""
    if HighestRankStarVisible then
        if HighestRankCfg.Type ~= ProtoRes.Game.pvp_rank_type.RT_None then
            HighestRankNameText = string.format(LSTR(130074), HighestRankName)
        end
    else
        HighestRankNameText = string.format(LSTR(130120), HighestRankName)
    end
    local VMList = {
        { Desc = LSTR(130015), Value = ModeStatistic.BattleCount },
        { Desc = LSTR(130016), Value = TimeStatistic.BattleCount },
        { Desc = LSTR(130017), Value = CurSeasonWinRateText },
        { Desc = LSTR(130018), Value = CurRankNameText, StarVisible = CurRankStarVisible, StarCount = CurRankWinStar },
        { Desc = LSTR(130019), Value = HighestRankNameText, StarVisible = HighestRankStarVisible, StarCount = HighestRankWinStar },
    } 
...
function PVPCrystallinePerformanceVM:UpdatePerformanceDetail()
    ...
    if BattleCount ~= 0 then
        ...
        local SurvivalDenominator = math.max(BattleCount, Death) -- 承伤分母用死亡数/局数，取决于哪个更大
        Survival = Survival / SurvivalDenominator
    end
