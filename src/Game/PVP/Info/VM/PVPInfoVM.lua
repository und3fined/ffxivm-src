...
local PVPTypeStatisticsVM = require ("Game/PVP/Info/VM/PVPTypeStatisticsVM")
...
function PVPInfoVM:ResetData()
	...
	self.CurCrystallineRank = nil
	self.SeasonHighestCrystallineRank = nil
	self.HasGetLaskSeasonRankReward = false
	self.CrystallineRankingInfoSelf = {}
	self.CrystallineRankingInfo = {}
	self.CrystallineRankRecordData = {
		-- [2] = {
		-- 	SeasonID = 3,
		-- 	RankID = 600,
		-- 	LikeNum = 10,
...
function PVPInfoVM:SetPVPOverviewData(Data)
	...
	self.CurCrystallineRank = Data.CurRankLvl
	self.SeasonHighestCrystallineRank = Data.CurSeasonMaxRankLvl
end
...
--- 获取当前水晶冲突段位
---@return uint32 段位ID
function PVPInfoVM:GetCurCrystallineRank()
	return self.CurCrystallineRank
end

--- 获取当前赛季水晶冲突最高段位
---@return uint32 段位ID
function PVPInfoVM:GetSeasonHighestCrystallineRank()
	return self.SeasonHighestCrystallineRank
end

--- 设置星里路标是否开启中
function PVPInfoVM:SetIsSeriesOpening(IsOpen)
	self.IsSeriesOpening = IsOpen
end
...
function PVPInfoVM:CheckNeedBreakthrough(SeriesData)
	...
	local NextLevelCfg = _G.PVPInfoMgr:GetSeriesMalmstoneLevelCfg(NextLevel)
...
---@param Info PvPColosseumSeasonRankRsp.Rank 协议数据
function PVPInfoVM:SetCrystallineRankingInfoSelf(SeasonID, Info)
	if self.CrystallineRankingInfoSelf then
		self.CrystallineRankingInfoSelf[SeasonID] = Info
	else
		FLOG_ERROR("[PVPInfoVM][SetCrystallineRankingInfoSelf]RankingInfo table nil")
	end
end
...
function PVPInfoVM:GetCrystallineRankingInfoSelf(SeasonID)
	return self.CrystallineRankingInfoSelf and self.CrystallineRankingInfoSelf[SeasonID]
end

--- 设置水晶冲突排行榜数据
---@param SeasonID uint32 赛季ID
---@param Info PvPColosseumSeasonRankRsp.Rank 协议数据
function PVPInfoVM:SetCrystallineRankingInfo(SeasonID, Info)
	if self.CrystallineRankingInfo then
		self.CrystallineRankingInfo[SeasonID] = Info
	else
		FLOG_ERROR("[PVPInfoVM][SetCrystallineRankingInfo]RankingInfo table nil")
	end
end
...
---@param SeasonID uint32 赛季ID
---@return PvPColosseumSeasonRankRsp.Rank 协议数据
function PVPInfoVM:GetCrystallineRankingInfo(SeasonID)
	return self.CrystallineRankingInfo and self.CrystallineRankingInfo[SeasonID]
end

--- 设置水晶冲突赛季记录数据
---@param SeasonID uint32 赛季ID
---@param Info PvPColosseumSeasonRankRsp.Rank 协议数据
function PVPInfoVM:SetCrystallineRankRecordData(SeasonID, Record)
	if self.CrystallineRankRecordData then
		self.CrystallineRankRecordData[SeasonID] = Record
	else
		FLOG_ERROR("[PVPInfoVM][SetCrystallineRankRecordData]Rank record table nil")
	end
end
...
function PVPInfoVM:GetCrystallineRankRecordData(SeasonID)
	return self.CrystallineRankRecordData and self.CrystallineRankRecordData[SeasonID]
end

--- 设置是否已领取水晶冲突上赛季段位奖励
---@param HasGetReward boolean 是否已领取
function PVPInfoVM:SetHasGetLaskSeasonRankReward(HasGetReward)
	self.HasGetLaskSeasonRankReward = HasGetReward
end

--- 获取是否已领取水晶冲突上赛季段位奖励
---@return boolean 是否已领取
function PVPInfoVM:GetHasGetLaskSeasonRankReward()
	return self.HasGetLaskSeasonRankReward
end

--- 获取是否参与过水晶冲突段位赛
---@return boolean 是否参与过
function PVPInfoVM:GetParticipatedCrystallineRank()
	if self.CrystallineRankRecordData then
		local CurVersionCfg = _G.PVPInfoMgr:GetCurVersionSeriesMalmstoneCfg()
		if CurVersionCfg then
			for _, Record in pairs(self.CrystallineRankRecordData) do
				-- 当前赛季数据也会拉取到，判断时跳过，只看以前赛季
				if Record.SeasonID ~= CurVersionCfg.SeasonID and Record.BtlNum > 0 then
					return true
				end
			end
		end
