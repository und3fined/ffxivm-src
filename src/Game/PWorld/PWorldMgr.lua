...
function PWorldMgr:SendLeavePWorld(PWorldID)
    ...
    if not CommonUtil.IsShipping() then
        FLOG_INFO("PWorldMgr:SendLeavePWorld \n"..debug.traceback())
    end
end
...
function PWorldMgr:OnEnterPWorld(MsgBody)
    ...
    elseif bChangeSameMap then
        FLOG_INFO("PWorldMgr:OnEnterPWorld change same map")
        ...
        FLOG_INFO("PWorldMgr:OnEnterPWorld maybe error")
        self:PostLoadWorld()
    end

    _G.EventMgr:SendEvent(EventID.PWorldEnterNotify)
...
function PWorldMgr:LeavePWorldImpl()
    ...
    local PWorldMgrInstance = CPWorldMgr:Get()
    PWorldMgrInstance:SetPWorldInstID(0)
    PWorldMgrInstance:SetPWorldResID(0)
    PWorldMgrInstance:SetMapResID(0)
...
function PWorldMgr:OnLeavePWorld(MsgBody)
    ...
    local LeavePWorldInstID = MsgBody.Leave.PWorldInstID
    if (self.BaseInfo.CurrPWorldInstID ~= LeavePWorldInstID) then
        FLOG_INFO("PWorldMgr:OnLeavePWorld LeavePWorldInstID is error, LeavePWorldInstID=%s, CurrPWorldInstID=%s", tostring(LeavePWorldInstID), tostring(self.BaseInfo.CurrPWorldInstID))
        return
    end
    self:LeavePWorldImpl()
end
...
function PWorldMgr:OnPWorldResult(MsgBody)
    --FLOG_INFO("PWorldMgr:OnPWorldResult MsgBody = " .. table.tostring_block(MsgBody))
...
function PWorldMgr:OnPWorldFinished(MsgBody)
    --FLOG_INFO("PWorldMgr:OnPWorldFinished MsgBody = " .. table.tostring_block(MsgBody))
...
function PWorldMgr:OnPWorldProgress(MsgBody)
    FLOG_INFO("PWorldMgr:OnPWorldProgress")
...
function PWorldMgr:OnPWorldTrans(MsgBody)
    ...
    if (CurrMapResID ~= PWorldTransRsp.MapResID) then
        ...
        local UWorldMgr = _G.UE.UWorldMgr:Get()
        if (UWorldMgr ~= nil) then
            --如果在野外大地图出现掉落，需要注意PWorldTransRsp.HideLoading是不是true，true表示短距离传送，不会加载子level
           local TransLoadType = UWorldMgr:CheckTransLoadType(
            ...
            if TransLoadType ~= _G.UE.EPWorldTransLoadType.Normal and (_G.StoryMgr:SequenceIsPlayingStatus() or _G.StoryMgr:SequenceIsPausedStatus()) then
                ...
                _G.FLOG_INFO("PWorldMgr:OnPWorldTrans SequenceIsPlaying or SequenceIsPaused!!!")
            end
            ...
            if (TransLoadType ~= _G.UE.EPWorldTransLoadType.Normal) then
                ...
                if not _G.StoryMgr:SequenceIsPlaying() and _G.UIViewMgr:IsViewVisible(_G.UIViewID.CommonFadePanel) then
                    local Params = {}
                    Params.FadeColorType = 1 -- 渐渐隐藏黑幕
                    Params.Duration = 1

                    local TargetView = _G.UIViewMgr:FindVisibleView(_G.UIViewID.CommonFadePanel)
                    if (TargetView ~= nil) then
                        TargetView:PlayFade(Params)
                    end
                end
            end
        end
...
function PWorldMgr:InternalReloadDynData()
    if (PWorldDynDataMgr) then
        PWorldDynDataMgr:Reset()
        PWorldDynDataMgr:Init()
        PWorldDynDataMgr:LoadDynDataAfterLoadWorldFinish()
    end
end
...
function PWorldMgr:OnPWorldCountDown(MsgBody)
	_G.UIViewMgr:HideView(_G.UIViewID.TeamBeginsCDWin)
    _G.SignsMgr:SetDuringCountDown(MsgBody.CountDown.EndTime ~= 0)
    ...
    _G.EventMgr:SendEvent(EventID.ExitRevertState)
end
...
---主城-房屋公共区域
function PWorldMgr:CurrIsInHousingPublic()
    local CurWorldSubType = self:GetCurrPWorldSubType()
    return CurWorldSubType == ProtoRes.pworld_sub_type.PWORLD_SUB_TYPE_HOUSE_PUBLIC
end

---副本-房屋室内
function PWorldMgr:CurrIsInHousingRoom()
    local CurWorldSubType = self:GetCurrPWorldSubType()
    return CurWorldSubType == ProtoRes.pworld_sub_type.PWORLD_SUB_TYPE_HOUSE_ROOM
end

---房屋
function PWorldMgr:CurrIsInHousing()
    return self:CurrIsInHousingPublic() or self:CurrIsInHousingRoom()
end
...
function PWorldMgr:IsNeedStreamingLevelInViewLocation()
    ...
    or PWorldTableCfg.Type == ProtoRes.pworld_type.PWORLD_CATEGORY_FIELD or PWorldTableCfg.Type == ProtoRes.pworld_type.PWORLD_CATEGORY_PRIVATE
    or PWorldTableCfg.Type == ProtoRes.pworld_type.PWORLD_CATEGORY_HOUSE) then
        return true
	end
...
---是否切换相同副本地图，包括切线、跨服等
function PWorldMgr:IsChangeSameMap()
    if (self.BaseInfo == nil) then
        return false
    end
    local Major = MajorUtil.GetMajor()
    ...
    if (self.BaseInfo.LastPWorldResID == self.BaseInfo.CurrPWorldResID and self.BaseInfo.LastMapResID == self.BaseInfo.CurrMapResID) then
        local Location = Major:FGetLocation(_G.UE.EXLocationType.ServerLoc)
        local Dist = _G.UE.FVector.Dist(self.MajorBirthPos, Location)
        local LimitDistance = 1000
        if Dist < LimitDistance then
...
---是否当前地图切线。PWorldResID和MapResID相同，LineID不同
function PWorldMgr:IsChangeLine()
    ...
    if (self.BaseInfo.LastPWorldResID == self.BaseInfo.CurrPWorldResID
...
---是否切换副本实例。与“IsChangeLine”的区别在于，副本销毁重新创建后，Line一样但是副本实例ID不一样
---@return boolean
...
function PWorldMgr:GetCurrWorldID()
	...
	if RoleVM then
        return RoleVM.CurWorldID ~= 0 and RoleVM.CurWorldID or RoleVM.WorldID
    else
...
--- 通过SgActor指针获取对应的InstanceID
---@param SgActor ASgLayoutActorBase
function PWorldMgr:FindSgActorInstanceID(SgActor)
    local SharedGroupComp = SgActor.SharedGroupLayoutComponent
    if not SharedGroupComp then
       return
    end

    local InstanceData = SharedGroupComp.InstanceData
...
--- 检查是否水晶冲突段位赛
function PWorldMgr:CurrIsInPVPColosseumRank()
    if self:CurrIsInPVPColosseum() then
        local PWorldID1fg = CrystallineParamCfg:FindCfgByKey(ProtoRes.Game.game_pvpcolosseum_params_id.PVPCOLOSSEUM_SCENEID_JLXX)
        local PWorldID2fg = CrystallineParamCfg:FindCfgByKey(ProtoRes.Game.game_pvpcolosseum_params_id.PVPCOLOSSEUM_SCENEID_HSZX)
        local PWorldID3fg = CrystallineParamCfg:FindCfgByKey(ProtoRes.Game.game_pvpcolosseum_params_id.PVPCOLOSSEUM_SCENEID_JXYS)
        if PWorldID1fg and PWorldID2fg and PWorldID3fg then
            local CurPWorldID = self:GetCurrPWorldResID()
            return CurPWorldID == PWorldID1fg.Value[2] or CurPWorldID == PWorldID2fg.Value[2] or CurPWorldID == PWorldID3fg.Value[2]
        end
    end
