...
---@class CommBackpack74SlotView : UIView
...
local CommBackpack74SlotView = LuaClass(UIView, true)

function CommBackpack74SlotView:Ctor()
...
function CommBackpack74SlotView:OnRegisterSubView()
	--AUTO GENERATED CODE 2 BEGIN, PLEASE DON'T MODIFY
	self:AddSubView(self.RedDot2)
	--AUTO GENERATED CODE 2 END, PLEASE DON'T MODIFY
end

function CommBackpack74SlotView:OnInit()
    UIUtil.SetIsVisible(self.RichTextLevel, false) 
end

function CommBackpack74SlotView:OnDestroy()

end

function CommBackpack74SlotView:OnShow()
...
function CommBackpack74SlotView:OnHide()
	self:StopAllAnimations()
    UIUtil.SetIsVisible(self.PanelAvailable, false)
end

function CommBackpack74SlotView:OnRegisterUIEvent()
...
function CommBackpack74SlotView:OnRegisterGameEvent()

end

function CommBackpack74SlotView:OnRegisterBinder()
    ...
    if (self.Binders == nil) then
        self.Binders = {
            ...
            {
                "Icon",
                ViewModel.bSyncIcon and UIBinderSetImageBrushSync.New(self, self.Icon) or UIBinderSetBrushFromAssetPath.New(self, self.Icon)
            },
            {
                "Num",
    ...
    ViewModel.ItemSlotType = ItemDefine.ItemSlotType.Item74Slot
    if ResID then
...
function CommBackpack74SlotView:OnClickButtonItem()
...
function CommBackpack74SlotView:SetClickButtonCallback(TargetView, TargetCallback)
    self.CallbackView = TargetView
    self.ClickCallback = TargetCallback
end

function CommBackpack74SlotView:OnDoubleClickButtonItem()
    local Params = self.Params
    if nil == Params then return end
...
function CommBackpack74SlotView:OnSelectChanged(IsSelected)
    if nil == self.Params then return end
...
function CommBackpack74SlotView:GetViewportPosition()
    local LocalPos = UIUtil.CanvasSlotGetPosition(self.Icon)
    local Pos = UIUtil.LocalToViewport(self.PanelInfo, LocalPos)
    return Pos
end

function CommBackpack74SlotView:SetIconImg(ImgName)
    UIUtil.ImageSetBrushFromAssetPath(self.Icon, ImgName)
end

function CommBackpack74SlotView:SetQualityImg(ImgName)
    UIUtil.ImageSetBrushFromAssetPath(self.ImgQuanlity, ImgName)
end

function CommBackpack74SlotView:SetNum(TargetNum)
    UIUtil.SetIsVisible(self.RichTextQuantity, true)
    self.RichTextQuantity:SetText(tostring(TargetNum))
end

function CommBackpack74SlotView:SetNumVisible(IsVisible)
    UIUtil.SetIsVisible(self.RichTextQuantity, IsVisible)
end

function CommBackpack74SlotView:SetRichTextLevelVisible(IsVisible)
    UIUtil.SetIsVisible(self.RichTextLevel, IsVisible)
end

function CommBackpack74SlotView:SetIsGet(IsGet)
    UIUtil.SetIsVisible(self.ImgMask, IsGet)
	UIUtil.SetIsVisible(self.IconReceived, IsGet)
end

function CommBackpack74SlotView:SetImgSelectVisible(IsVisible)
    UIUtil.SetIsVisible(self.ImgSelect, IsVisible)
end

function CommBackpack74SlotView:SetIconChooseVisible(IsVisible)
    UIUtil.SetIsVisible(self.IconChoose, IsVisible)
end

function CommBackpack74SlotView:SetRewardShow(IsVisible)
    UIUtil.SetIsVisible(self.PanelAvailable, IsVisible)
end

return CommBackpack74SlotView