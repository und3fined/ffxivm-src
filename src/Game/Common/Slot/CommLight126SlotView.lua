...
function CommLight126SlotView:OnInit()
	self.Binders = {
		...
		{ "IsWearable", UIBinderSetIsVisible.New(self, self.ImgWearable)},
	}
end
...
function CommLight126SlotView:OnShow()
	...
	if  not string.isnilorempty(self.ViewModel.RedDotName) then
		...
		if self.ViewModel.RedDotStyle then
			self.RedDot:SetStyle(self.ViewModel.RedDotStyle)
		end
	else
...
function CommLight126SlotView:OnClickButtonItem()
...
function CommLight126SlotView:SetClickButtonCallback(TargetView, TargetCallback)
...
function CommLight126SlotView:OnSelectChanged(bSelect)
	local ViewModel = self.Params and self.Params.Data
	if ViewModel == nil then return end

	if ViewModel.OnSelectChanged then
		ViewModel:OnSelectChanged(bSelect)
	end
end

function CommLight126SlotView:OnItemSelectChanged(bSelect)
