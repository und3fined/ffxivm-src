...
local RoundControlCfg = require("TableCfg/OutOnALimbLoopControlCfg")
...
local DynAssetInstanceID = 5965779 -- 关卡中创建的动态物件实例id
...
local MiniGameOutOnALimb = LuaClass(MiniGameBase)

---Ctor
function MiniGameOutOnALimb:Ctor()
    local Type = MiniGameType.OutOnALimb
	self.MiniGameType = Type
    self.Name = MiniGameClientConfig[Type].Name
    self.UIViewMainID = UIViewID.OutOnALimbMainPanel
    self.DoubleWinViewID = UIViewID.OutOnALimbDoubleWin
    self.SettlementViewID = UIViewID.OutOnALimbSettlementPanel
    self.IdleStateKey = AnimTimeLineSourceKey.FellingIdle
...
--- 初始化游戏数据
function MiniGameOutOnALimb:LoadTableCfg()
...
--- 初始化游戏数据
function MiniGameOutOnALimb:InitMiniGame()
...
--- 设定选择游戏难度MsgText内容
function MiniGameOutOnALimb:OnCreateDifficultyParams()
...
--- 是否有难度选择阶段
function MiniGameOutOnALimb:OnIsGameHaveDifficultySelect()
    return true
end

--- 获取镜头移动参数
function MiniGameOutOnALimb:OnCreateCameraSettingParam()
...
--- 是否结束游戏
function MiniGameOutOnALimb:OnIsGameRunEnd()
	...
	if RemainTime <= 0 and not self.bActLockForTime then
...
--- 是否带有翻倍阶段
function MiniGameOutOnALimb:OnIsHaveRestartStage()
    local GameType = self.MiniGameType
...
--- 翻倍挑战提示参数
function MiniGameOutOnALimb:OnCreateRestartContentParams()
...
--- 获取游戏翻倍挑战重置时间和次数
function MiniGameOutOnALimb:GetRestartTimeAndChances()
	local Time = 0
	local Chances = 0
	local DCfg = self.DefineCfg
    if DCfg then
...
--- 获取当前轮能够获得的奖励
function MiniGameOutOnALimb:GetTheRewardGotInTheRoundInternal(Round)
...
--- 获取当前轮能够获得的奖励
function MiniGameOutOnALimb:GetTheRewardGotInTheRound()
	local CurRound = self:GetRoundIndex()
    return self:GetTheRewardGotInTheRoundInternal(CurRound)
end

--- 获取当前轮轮盘速度
function MiniGameOutOnALimb:GetTheSpeedInTheRound()
	local DifficultyLv = self:GetDifficulty()
	local DifficultyCfg = DifficultControlCfg:FindCfgByKey(DifficultyLv)
	if DifficultyCfg == nil then
		return
	...
	if RoundCfg then
		...
		FLOG_ERROR("MiniGameOutOnALimb:GetTheSpeedInTheRound:RoundIndex is valid. CurRound:%s", tostring(CurRound))
	end
    FLOG_INFO("MiniGameOutOnALimb:GetTheSpeedInTheRound Difficulty: %s, CurRound: %s, ActualSpeed: %s", tostring(DifficultyLv), tostring(CurRound),tostring(ActualSpeed))
    return ActualSpeed
end

--- 获取顶部tips内容
function MiniGameOutOnALimb:OnGetTopTipsContent(GameState)
    local Title
    local Content
...
--- 退出时重置游戏信息（for再战）
function MiniGameOutOnALimb:Reset()
    self:ResetGameInfo()
end

--- 播放主角砍伐/挖掘动画
function MiniGameOutOnALimb:PlayMajorCutAnimation()
    ...
    local ReIdleTimer = _G.TimerMgr:AddTimer(self, RePlayIdleAnim, IdleAnimPlayAheadTime)
    self:AddTimerHandle(ReIdleTimer)
end

return MiniGameOutOnALimb