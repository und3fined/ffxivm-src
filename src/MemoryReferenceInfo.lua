...
local function FormatDateTimeNow()
    ...
    --jianweili
    --local strDateTime = tostring(_G.TimeUtil.GetLocalTime() * 1000 + _G.TimeUtil.GetLocalTimeMS())
    local strDateTime = string.format("%02d%02d%02d", tostring(cDateTime.hour), tostring(cDateTime.min), tostring(cDateTime.sec))
    local strStamp = tostring(_G.TimeUtil.GetLocalTimeMS())
    strDateTime = strDateTime.."-"..strStamp.sub(strStamp, string.len(strStamp) - 2)
...
local function CreateObjectReferenceInfoContainerFromFile(strFilePath)
	...
	for strLine in cFile:lines() do
		...
		if "--" ~= strHeader then
			local _, _, strAddr, strName, strRefCount= string.find(strLine, "(.+)"..GapString.."(.*)"..GapString.."(%d+)")
			if strAddr then
...
local function OutputMemorySnapshot(strSavePath, strExtraFileName, nMaxRescords, strRootObjectName, cRootObject, cDumpInfoResultsBase, cDumpInfoResults)
	...
	table.sort(cRes, function (l, r)
        ...
        elseif type(l) < type(r) then
            ...
            local strL = tostring(l)
            local strR = tostring(r)
            if strL > strR then
                return true
            elseif strL < strR then
                return false
            else
                return cRefInfo[l] > cRefInfo[r]
            end
	...
	if bOutputFile then
		...
		-- Combine file name.
		strFileName = strSavePath .. "DumpLuaMemory"
	...
	if cDumpInfoResultsBase then
		...
		cOutputer("--------------------------------------------------------\n")
		cOutputer("-- base file (A):   " .. cDumpInfoResultsBase.m_strShortSrc .. "\n")
		cOutputer("-- compared file (B):   " .. cDumpInfoResults.m_strShortSrc .. "\n")
        cOutputer("-- result = B - A \n")
	else
		cOutputer("--------------------------------------------------------\n")
	...
	cOutputer("--------------------------------------------------------\n")
	cOutputer("[Type And Address/Cotent]           [Reference Path]           [Reference Count]\n")
	--cOutputer("--------------------------------------------------------\n")
			...
			if (nMaxRescords > 0) then
				if (i <= nMaxRescords) then
                    if "string" == type(v) then
                        ...
                        if ((not cDumpInfoResultsBase) and ((nil == nPattenBegin) or (nil == nPattenEnd))) then
                            local strRepString = string.gsub(strOrgString, "([\n\r])", "\\n")
                            cOutputer("string: \"" .. strRepString .. "\"" .. GapString  .. cNameInfo[v] .. GapString .. tostring(cRefInfo[v]) .. "\n")
                        else
                            cOutputer(tostring(v) .. GapString .. cNameInfo[v] .. GapString .. tostring(cRefInfo[v]) .. "\n")
                        end
                    else
                        cOutputer(GetOriginalToStringResult(v) .. GapString .. cNameInfo[v] .. GapString .. tostring(cRefInfo[v]) .. "\n")
                    end
				end
			else
                if "string" == type(v) then
...
local function DumpMemorySnapshot(strSavePath, strExtraFileName, nMaxRescords, strRootObjectName, cRootObject)
	...
	if cRootObject then
        ...
        --jianweili
        --use _G or debug.getregistry() as root
