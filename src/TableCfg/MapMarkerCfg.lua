...
function MapMarkerCfg:GetAllMarkerCfg(Marker)
	local SearchConditions = string.format("Marker = %d AND Hide=0", Marker)
	return self:FindAllCfg(SearchConditions)
end

---GetAllMarkerCfgByEventType @获取某个地图的事件类型为EventType的所有Marker
---@param Marker number
---@param EventType number
function MapMarkerCfg:GetAllMarkerCfgByEventType(Marker, EventType)
